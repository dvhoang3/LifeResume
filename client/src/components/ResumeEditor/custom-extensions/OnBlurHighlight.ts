            ...
            blur: view => {
              ...
              rects.forEach(r => {
                const overlap = lines.find(
                  l => !(r.bottom < l.top || r.top > l.bottom)
                );
                if (overlap) {
              ...
              lines.forEach(l => {
                ...
                Object.assign(o.style, {
                  position: 'absolute',
                  top: `${l.top + root.scrollTop - rootBox.top - EXTRA_SPACE}px`,
                  left: `${l.left + root.scrollLeft - rootBox.left}px`,
                  width: `${l.right - l.left}px`,
                  height: `${l.bottom - l.top + EXTRA_SPACE}px`,
                  background: 'rgba(141, 141, 141, 0.33)',
                  borderRadius: '2px',
                  pointerEvents: 'none',
                  zIndex: '5',
                });
